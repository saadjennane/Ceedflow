-- Transactional email templates: auto-sent on triggers (e.g. form submitted)
-- or launched manually from an application / a bulk selection.
-- Each template has an on/off toggle (`enabled`) and bilingual content (FR + EN).
...
CREATE TABLE email_templates (
  ...
  name TEXT NOT NULL,                        -- human label (admin UI)
  description TEXT,
  trigger_event email_template_trigger NOT NULL DEFAULT 'manual',
  enabled BOOLEAN NOT NULL DEFAULT true,     -- the toggle
  is_internal BOOLEAN NOT NULL DEFAULT false,-- true => sent to admins, not the applicant

  -- Bilingual content (markdown body, {{variables}} substituted at send time)
...
-- Log of every transactional template send (distinct from bulk campaign sends)
CREATE TABLE email_template_sends (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE INDEX idx_email_template_sends_template ON email_template_sends(template_key);
CREATE INDEX idx_email_template_sends_application ON email_template_sends(application_id);
CREATE INDEX idx_email_template_sends_sent_at ON email_template_sends(sent_at DESC);
...
-- ---------------------------------------------------------------------------
-- Seed: the two automatic emails (form submitted) + four manual templates.
-- Bodies use minimal markdown; {{founder_first_name}}, {{startup_name}},
-- {{stage}}, {{sector}}, {{review_url}} are substituted at send time.
-- ---------------------------------------------------------------------------
