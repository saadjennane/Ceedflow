-- Rework ratings: 1 score per sub-criterion, 1 comment per criterion
...
CREATE TABLE application_ratings (
  ...
  UNIQUE (application_id, admin_id, criterion, sub_index)
);
...
CREATE TABLE application_rating_comments (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  application_id UUID NOT NULL REFERENCES applications(id) ON DELETE CASCADE,
  admin_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  criterion TEXT NOT NULL CHECK (criterion IN (
...
CREATE INDEX idx_application_rating_comments_app ON application_rating_comments(application_id);

ALTER TABLE application_rating_comments ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Authenticated users can manage rating comments"
  ON application_rating_comments FOR ALL TO authenticated USING (true) WITH CHECK (true);

CREATE TRIGGER application_rating_comments_updated_at
  BEFORE UPDATE ON application_rating_comments
  FOR EACH ROW EXECUTE FUNCTION update_updated_at();
