...
export type SendTemplateTarget = ApplicationTarget | JurorTarget
...
export default function SendTemplateModal({ target, templates, fromAddresses, onClose }: Props) {
  ...
  const recipientLabel = target.kind === 'application'
    ? `${target.founderName} · ${target.founderEmail}`
    : `${target.firstName} ${target.lastName} · ${target.email}`

  const recipientEmail = target.kind === 'application' ? target.founderEmail : target.email

  const ctx: TemplateContext = useMemo(() => {
    if (target.kind === 'application') {
      return {
        founder_name: target.founderName,
        founder_first_name: target.founderName.trim().split(/\s+/)[0] || '',
        startup_name: target.startupName,
        stage: target.stage,
        sector: target.sector,
      }
    }
    return {
      juror_name: `${target.firstName} ${target.lastName}`.trim(),
      juror_first_name: target.firstName,
      juror_role: target.role,
    }
  }, [target])
  ...
  const send = async () => {
    ...
    if (target.kind === 'application') payload.applicationId = target.applicationId
    else payload.jurorId = target.jurorId
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-3xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
          ...
          <div className="flex-1 overflow-y-auto p-5">
            {templates.length === 0 ? (
              <div className="text-center py-12 text-sm text-gray-500">
                Aucun template manuel actif pour ce type de destinataire.<br />
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-4">
            <div className="flex items-center flex-wrap gap-2">
              ...
              <div className="flex items-center gap-0.5 bg-gray-100 border border-gray-200 rounded-full p-0.5">
                <button onClick={() => switchLanguage('fr')} className={`px-3 py-0.5 text-xs rounded-full ${language === 'fr' ? 'bg-emerald-500 text-white' : 'text-gray-600'}`} disabled={sending}>FR</button>
                <button onClick={() => switchLanguage('en')} className={`px-3 py-0.5 text-xs rounded-full ${language === 'en' ? 'bg-emerald-500 text-white' : 'text-gray-600'}`} disabled={sending}>EN</button>
              </div>
            ...
            {fromAddresses.length > 1 ? (
              <div>
                <label className="block text-xs font-medium mb-1">Expéditeur</label>
                <select
                  value={fromAddress}
                ...
                >
                  {fromAddresses.map(a => <option key={a} value={a}>{a}</option>)}
                </select>
              </div>
            ) : (
              <div className="text-xs text-gray-500">Expéditeur : <strong>{fromAddresses[0] || 'défaut'}</strong></div>
            )}
        ...
        {step === 'compose' && selected && (
          <div className="px-5 py-3 border-t border-gray-200 flex items-center justify-between">
            <p className="text-xs text-gray-500">Sera envoyé à <strong>{recipientEmail}</strong></p>
